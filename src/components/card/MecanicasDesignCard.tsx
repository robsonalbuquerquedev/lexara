...
import { mecanicasDesignArticles } from "@/data/mecanicasDesignArticles";

export default function MecanicasDesignCard() {
    return (
        <section className="max-w-6xl mx-auto px-6 py-16">
            ...
            <header className="mb-12">
                ...
                <p className="text-muted-foreground max-w-3xl">
                    Gameplay na veia: sistemas, progressão, combate, level design e por que
                    certos jogos viciam (no bom sentido).
                </p>
            </header>

            {/* Cards */}
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {mecanicasDesignArticles.map((article) => (
                    <Link
                        key={article.id}
                        href={article.href}
